  ...
  useEffect(() => {
    if (!loaded && user) {
      void (async () => {
        ...
        const recentDecksPromises: Promise<DeckInterface>[] | undefined =
          user.decks?.map((idDeck) => fetchDeckById(idDeck));
        ...
        if (decks.length === 0) {
          ...
          const recommendedDecksPromises:
            | Promise<DeckInterface[]>[]
            | undefined = recentTags.map((tag) => fetchDeckByTag(tag));
          ...
          recoDecks = recoDecks.filter(
            (deck) => !decks.some((deckRecent) => deckRecent._id === deck._id)
          );
