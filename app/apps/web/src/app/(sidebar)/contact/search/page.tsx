        ...
        if (!loaded) {
            ...
            if (session?.user) {
                if (session.user.email) {
                    ...
                    dataPromise.then((data: UserInterface[]): void => {

                        // @ts-ignore
                        const user = data.find(e => e.email === session.user.email)
                        // @ts-ignore
                        const dataFilter = data.filter(e => e.email !== session.user.email && !user?.following?.includes(e.email))
    ...
    return (
        <div className="p-[5vh]">
            ...
            <div className="mt-10 flex justify-center">
                <ResearchBar onChange={handleSearch} placeholder="Rechercher un contact par son nom, prénom, pseudo"/>
            </div>
