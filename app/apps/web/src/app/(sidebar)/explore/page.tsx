...
export default function Page(): JSX.Element {
  ...
  function tagsOnClick(event: React.MouseEvent<HTMLButtonElement>): void {
    ...
    console.log(tagName);
    setDecks(allDecks);
    setDecks(allDecks.filter((deck) => deck.tags.includes(tagName)));
  }
  ...
  decks.sort(
    (deck1, deck2) =>
      deck2.votes.up - deck2.votes.down - (deck1.votes.up - deck1.votes.down)
  );

  for (const deck of decks) {
    if (deck.isPublic) {
      listeDecks.push(
        <DeckUI deck={deck} key={deck._id.toString()} type="import" />
      );
    }
  }
  ...
  for (const tag of allTags) {
    listeTags.push(
      <CarouselItem className="md:basis-1/6 lg:basis-1/12" key={tag.title}>
        <button name={tag.title} onClick={tagsOnClick} type="button">
          <Tag title={tag.title} />
        </button>
      </CarouselItem>
    );
      ...
      <div className="flex flex-col items-center space-y-10 p-10">
        ...
        <div className="flex flex-col size-full items-center">
          <Carousel className="w-[75%]" id="carousel" opts={{ align: "start" }}>
            <CarouselContent>{listeTags}</CarouselContent>
            <CarouselPrevious />
            <CarouselNext />
          </Carousel>
        </div>
        {loaded ? (
          <div className="flex flex-wrap w-full justify-center gap-3">
            ...
            >
              <h1 className="font-Lexend text-xl font-medium">
                Importer le deck
              </h1>
              <div className="flex flex-col space-y-5 items-center mt-5">
                <p className="text-center font-Lexend text-sm">
                  Ce deck sera ajouté dans votre collection Mes decks. Vous
                  pourrez modifier le contenu des cartes.
                </p>
                <button
                  className="px-5 py-2 bg-black text-white rounded-full font-Lexend w-fit"
                  onClick={() => {
                    setIsImportOpen(false);
                  }}
                  type="button"
