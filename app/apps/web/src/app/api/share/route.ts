...
export async function POST(req: NextRequest): Promise<NextResponse> {
    ...
    for (const contact of data.contactData) {
        if (contact.selected) {
            ...
            User.findOneAndUpdate({_id: new Types.ObjectId( contact.userId)}, {$pull: {sharedDecks: {deck_id: data.deckId}}}).catch(() => {
                return NextResponse.json({success: false} as ShareResponse, {status: 500});
            });
        }
    }

    await Deck.findOneAndUpdate({_id: new Object(data.deckId)}, {$set: {sharedTo}});
