...
export async function GET(req: NextRequest, {params}: { params: { userId: string } }): Promise<NextResponse> {
    ...
    const user: UserInterface | null = await User.findOne({_id: new ObjectId(userId)});
    ...
    for (const sharedDeck of user.sharedDecks) {
        const deck: DeckInterface | null = await Deck.findOne({_id: new ObjectId(sharedDeck.deck_id)});
        if (deck) {
            decks.push(deck);
        }
    }
