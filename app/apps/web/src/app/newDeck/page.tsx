...
import {createRef, Dispatch, MutableRefObject, RefObject, SetStateAction, useRef, useState} from "react";
...
const CardEditor = (card: Card, cards: Card[], setCards: Dispatch<SetStateAction<Card[]>>): JSX.Element => {
    ...
    const autoGrow = (ref: MutableRefObject<any>) => {
        ...
        card.question = questionRef.current.value;
        card.reponse = reponseRef.current.value;
    ...
    return <div className="bg-white rounded-md" key={card.id_card}>
        <div className="flex justify-between px-5 pt-5">
            ...
            <div className="flex space-x-1">
                ...
                <img onClick={deleteCard} src="/delete.svg" alt=""/>
            </div>
        </div>
        <hr className="my-2"/>
        ...
        <div className="flex space-x-3">
            <div className="flex flex-col justify-between px-5 pb-5 grow">
                    ...
                    }} ref={questionRef} defaultValue={card.question}></textarea>
            ...
            <div className="flex flex-col justify-between px-5 pb-5 grow">
                    <textarea className="resize-none h-auto" onChange={() => {
                        autoGrow(reponseRef);
                    }} ref={reponseRef} defaultValue={card.reponse}></textarea>
                <hr className="border-2 border-black my-2"/>
...
function Page(): JSX.Element {
    ...
    const cardsJSX = cards.map((c) => {
        return CardEditor(c, cards, setCards);
    });

    const addCard = () => {
        setCards([...cards, {
            id_card: cards.length + 1,
            question: "",
            reponse: "",
            palier: 0,
            derniereRevision: 0,
