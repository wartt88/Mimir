...
export default function LoginForm(): JSX.Element {
  ...
  if (session?.user?.email) {
    ...
    promiseUsr
      .then((user: UserInterface | null) => {
        if (user === null) {
          signOut({ redirect: false, callbackUrl: "/register" })
            .then((res) => {
  ...
  const handleSubmit = async (e: FormEvent): Promise<void> => {
    ...
    try {
      ...
      if (res?.error) {
        setError(
          "L'adresse e-mail ou le mot de passe n'est pas valide, veuillez réessayer"
        );
        return;
      }
          ...
          <div className="w-full lg:min-w-[50%] h-[100vh] bg-white font-Lexend flex flex-col items-center">
            <div className="flex py-5 w-3/4  mt-16">
              <div className="grow space-x-20 text-2xl">
                <Link className="underline underline-offset-8" href="/login">
                  Se connecter
                </Link>
            ...
            >
              ...
              <input
                ...
                type="password"
              />
              <Link className="self-end text-blue-500" href="/reset">
                Mot de passe oublié
              </Link>
              <button
...
function ProviderButton({ name }: { name: string }): JSX.Element {
  ...
  return (
    <button
      className="border-2 border-gray-400 text-gray-400 p-3 rounded-md text-md text-center"
      onClick={handleClick}
      type="button"
    >
